...
| 包管理 / 运行 | uv 或 poetry + uvicorn；Cocos 编辑器 | 后端与客户端各自独立 |
...
| 任务 | 产出 |
|------|------|
| 创建 Cocos 3 工程于 `client/` | 浏览器可 Preview |
| 搭建 FastAPI 骨架于 `server/` | `GET /health` 可用 |
| 第三人称跟拍原型 | 角色前后移动时相机跟随，带轻度雾 |
| 平台适配空壳 | `platform/web.ts` / `platform/wechat.ts` |
| 文档与规范 | 本文档 + ESLint/Ruff 等基础配置 |
...
| 幕 | 任务 |
|----|------|
| A | 废弃列车猪头怪演出（可见数秒）；稻草人仪式（约 3 秒后消失） |
