...
**目标**：弱联网闭环 + 可提交体验版。  
**进度**：✅ 后端与 web 存档闭环可验收；微信构建待 Creator 本机执行。详见 [`阶段3-验收说明.md`](./阶段3-验收说明.md)。

| 任务 | 产出 | 状态 |
|------|------|------|
| 本地存档 | 关卡进度、检查点、设置 | ✅ |
| FastAPI：微信登录 / 测试登录 | JWT | ✅ |
| 云存档同步 | 版本号乐观锁，冲突服务器为准 | ✅ |
| 远程配置 | 难度、VO、cdnBaseUrl、功能开关 | ✅ |
| 微信小游戏构建 | 分包、首包说明 | ⏳ 文档就绪，待 Creator |
| CDN | 配置下发 + 外置约定 | ✅ |
...
| 方向 | 内容 | 状态 |
|------|------|------|
| 手感 | 后坐力、受击硬直、镜头震动、难度曲线 | ✅ 后坐力/震动/受击反馈；难度曲线持续 |
| 表现 | 雾、脚步、环境音、死亡演出；战术人形+步枪；木箱掩体 | ✅ 胶囊肢体+布料纹理人形；采样枪声；Edge 神经 TTS |
| 语音 / 音效 | 对讲机与进度播报、枪/脚步 | ✅ `/api/v1/tts` + `audio/*.wav` |
| 性能 | Draw call、阴影策略、微信真机帧率 | ⏳ 阴影 map 收敛；真机待 Creator |
| 关卡 2+ | 新异象、新敌人规则（仍保持弱联网） | ⏳ |
| 运营 | 简易排行榜、崩溃日志、配置热更 | ⏳ |

视觉标准：`assets/view-pseudo-3d.png`（第三人称战术、雾林、木箱）。  
人物：优先加载 `client/web-preview/models/Soldier.glb`（蒙皮士兵），失败回退程序化胶囊人形。  
语音：左下角进度卡 + 左上角对讲机 → Edge 神经 TTS（需后端）；枪声为本地 WAV 采样。
...
| `docs/阶段3-验收说明.md` | 已完成 | 登录/存档/配置/微信构建说明 |
